...
if (!isset($_SESSION['email'])) {
    die("<script>alert('Silakan login terlebih dahulu.'); window.location.href = '../login.php';</script>");
}
...
$query_role = "SELECT role FROM users WHERE email = ?";
$stmt_role = $conn->prepare($query_role);
if (!$stmt_role) {
    die("Error dalam query SQL: " . $conn->error);
}
$stmt_role->bind_param("s", $_SESSION['email']);
$stmt_role->execute();
...
if ($row_role = $result_role->fetch_assoc()) {
    ...
    if ($role !== 'penyelenggara') {
        die("<script>alert('Anda tidak memiliki akses ke dashboard ini.'); window.location.href = '../login.php';</script>");
    }
} else {
    die("<script>alert('Pengguna tidak ditemukan.'); window.location.href = '../login.php';</script>");
}

// Deklarasi array queries
$queries = [
    "total_event" => "SELECT COUNT(*) AS total_event FROM events WHERE admin_email = ?",
    "total_peserta" => "SELECT COUNT(*) AS total_peserta FROM peserta WHERE event_id IN (SELECT event_id FROM events WHERE admin_email = ?)",
    "total_sertifikat" => "SELECT COUNT(*) AS total_sertifikat FROM sertifikat WHERE event_id IN (SELECT event_id FROM events WHERE admin_email = ?)"
];
...
// Ambil hasil dari array
$total_event = $results['total_event'] ?? 0;
$total_peserta = $results['total_peserta'] ?? 0;
$total_sertifikat = $results['total_sertifikat'] ?? 0;
...
<body class="bg-gray-100 font-sans">
    <div class="flex h-screen">
        ...
        <div class="w-64 bg-blue-600 text-white">
            <div class="p-4 text-2xl font-bold">Sikilat Dashboard</div>
            <nav>
                <button onclick="showSection('dashboard')" class="block w-full text-left py-2 px-4 hover:bg-blue-700">Dashboard</button>
                <button onclick="showSection('acara-saya')" class="block w-full text-left py-2 px-4 hover:bg-blue-700">Acara Saya</button>
                <button onclick="showSection('daftar-peserta')" class="block w-full text-left py-2 px-4 hover:bg-blue-700">Data Peserta</button>
                <button onclick="showSection('rekap-kehadiran')" class="block w-full text-left py-2 px-4 hover:bg-blue-700">Rekap Kehadiran</button>
                <button onclick="showSection('pengaturan')" class="block w-full text-left py-2 px-4 hover:bg-blue-700">Pengaturan</button>
            </nav>
        </div>
        ...
        <div class="flex-1 p-6">
            ...
            <div id="dashboard" class="section">
                <h1 class="text-3xl font-bold mb-6">Dashboard Penyelenggara</h1>
                <div class="grid grid-cols-3 gap-6">
                    <div class="bg-white p-4 shadow rounded-lg">
                        <h2 class="text-lg font-semibold">Total Acara</h2>
                        <p class="text-2xl font-bold"><?php echo $total_event; ?></p>
                    </div>
                    <div class="bg-white p-4 shadow rounded-lg">
                        <h2 class="text-lg font-semibold">Peserta Terdaftar</h2>
                        <p class="text-2xl font-bold"><?php echo $total_peserta; ?></p>
                    </div>
                    <div class="bg-white p-4 shadow rounded-lg">
                        <h2 class="text-lg font-semibold">Sertifikat Terkirim</h2>
                        <p class="text-2xl font-bold"><?php echo $total_sertifikat; ?></p>
                    </div>
                </div>
            </div>

            <!-- Acara Saya Section -->
            <div id="acara-saya" class="section hidden">
                <h1 class="text-3xl font-bold mb-6">Acara Saya</h1>
                <button onclick="openModal('addModal')" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">Tambah Event</button>
                <!-- Daftar acara akan diisi secara dinamis -->
                                    <!-- Tabel Event -->
            ...
            <!-- Buat Acara Section -->
            <div id="daftar-peserta" class="section hidden">
                <h1 class="text-3xl font-bold mb-6">Buat Acara Baru</h1>
...
<div id="daftar-peserta" class="section hidden">
    ...
    <div class="size-full w-full max-w-screen-lg p-4 mt-4">
        <table id="pesertaTable" class="md:table-fixed w-full">
            ...
            <div id="rekap-kehadiran" class="section hidden">
                <h1 class="text-3xl font-bold mb-6">Rekap Kehadiran</h1>
                <p>Data kehadiran peserta akan ditampilkan di sini.</p>
            </div>

            <!-- Pengaturan Section -->
            <div id="pengaturan" class="section hidden">
                <h1 class="text-3xl font-bold mb-6">Pengaturan</h1>
                <p>Pengaturan akun dan preferensi lainnya.</p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function createEvent() {
            Swal.fire({
                ...
                html: '<input type="text" id="eventName" class="swal2-input" placeholder="Nama Acara">' +
                ...
                preConfirm: () => {
                    ...
                    if (!eventName || !eventDate) {
                        ...
                        Swal.fire('Berhasil!', 'Acara baru telah dibuat.', 'success');
                    }
                }
            });
        }
        ...
        function openEditModal(event) {document.getElementById('event_id').value = event.event_id;document.getElementById('event_name').value = event.event_name;document.getElementById('event_date').value = event.event_date;document.getElementById('start_time').value = event.start_time;document.getElementById('end_time').value = event.end_time;document.getElementById('description').value = event.description;
        openModal('EditModal');}
